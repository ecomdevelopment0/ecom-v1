...
const intitiatePaymentIndividual = async (userId: string, data: any) => {
  return await paymentProvider.intitiatePaymentIndividual(userId, data);
};
...
export default function Checkout() {
  ...
  const quantity: number = Number(query.get("quantity"));
  const subTotal: number = quantity * (product?.marketPrice || 0);
  ...
  const pay = async () => {
    ...
    const intitiatePaymentIndividualRes = await intitiatePaymentIndividual(
      authData?.userId,
      {
        totalAmount,
        productId,
    ...
    if (intitiatePaymentIndividualRes.success) {
      const paymentResponse = intitiatePaymentIndividualRes.data;
      let orderRes: any;
      if (paymentResponse) {
        const options: any = {
      ...
      Swal.fire({
        title: "Oops ...!",
        text: intitiatePaymentIndividualRes.message,
        icon: "error",
        confirmButtonColor: "#337CCF",
      });
      navigate("/");
